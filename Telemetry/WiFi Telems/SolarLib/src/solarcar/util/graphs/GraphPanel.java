...
public class GraphPanel extends JPanel implements Runnable, HierarchyListener {
    ...
    public GraphPanel(int timeLength, int messageFrequency, int updateFrequency,
        ...
        this.midlimit = Integer.toString((int) (ymax + ymin) / 2);
	...
	public synchronized void start() {
		...
		thread = new Thread(this, "GraphPanel" + count);
		count++;
		thread.start();
	}
    ...
    public void paintComponent(Graphics g) {
		...
		/*
		 * Allows us to get the value once and then use it throughout the code 
		 * without worrying about it changing and without having to worry about 
        ...
        x1 = size.width - ((tScale) * (time - pts[(curPtLocal + 1) % pts.length].time)) + xoffset;
        y1 = size.height - (yScale * pts[(curPtLocal + 1) % pts.length].value);

        //System.out.println("drawing\t" + time + "\t" + size.width + "\t" + tScale+ "\t" + time);

        g2.setColor(col);

        for (int i = (curPtLocal + 1) % pts.length; i != curPtLocal - 2; i = (i + 1) % pts.length) {
