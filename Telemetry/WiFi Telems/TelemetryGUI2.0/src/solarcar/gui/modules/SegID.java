...
public class  SegID extends SolarPanel implements Runnable {
    ...
    @Override
    public void init() {
        setBorder(new TitledBorder("SegID"));
        setLayout(new GridBagLayout());
//        setPreferredSize(new Dimension(90, 65));
...
//        endCond = new JLabel("End Condition: N/A ");
        gradeLabel = new JLabel("Grade: N/A %");
...
//        add(speedlimit, c);
    ...
    private void updateData() {
        ...
        gradeLabel.setText("Grade: " + (df.format((Double) route_grade.get(smallest_idx))) + " %");
...
//		VDCConn.get().sendMessage("chat msg='" + segID + " ' id='Seg ID:'");
		...
		gradeLabel.setText("Grade: " + (df.format((Double) grade)) + " %");
                if ((Double) route_grade.get(smallest_idx) >= 1.5) {
                    gradeLabel.setForeground(Color.RED);
                } else if ((Double) route_grade.get(smallest_idx) < -1.5) {
                    gradeLabel.setForeground(Color.GREEN);
